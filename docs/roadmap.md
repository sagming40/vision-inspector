...
## M1 — Python 프로토타입 + 성능 측정  `2~3일` ✅ 완료 (2026.07.26)

**이 단계를 건너뛰면 프로젝트 전체의 명분이 사라진다.**

- [x] Python + OpenCV로 표준 파이프라인 구현 (그레이스케일 → 블러 → 이진화 → 모폴로지)
- [x] 블롭 검출·면적 계산을 A(순수 반복문) / B(numpy 벡터화) 두 방식으로 각각 구현
- [x] 벤치마크 하네스(워밍업10/실측100) 구현 및 sanity check 검증
- [x] `docs/benchmark.md`에 단계별 실측치 기록

**완료 조건:** "Python 단독으로는 N fps" 라는 숫자가 문서에 적혀 있음 ✅
**초과 달성:** 원래 계획(반전/엣지검출)보다 범위를 넓혀, ADR-005 핵심 논거인
⑧번 블롭 검출을 A/B 두 가지로 구현·비교. numpy(B)가 순수 반복문(A)보다
**15배 느리다는 결과**까지 확인 — "느린 쪽만 골라 비교했다"는 반박을 선제 차단.

---

## M2 — 국경 넘기 (C++ DLL ↔ C# P/Invoke)  `3~5일` ✅ 완료 (2026.07.26)

- [x] C++ 에 `Add(int,int)` 하나만 export
      → `extern "C"` + `__declspec(dllexport)` + `__stdcall`
- [x] `dumpbin /exports` 로 이름이 안 뭉개졌는지 눈으로 확인
- [x] C# 에서 `[DllImport]` 로 호출 → **`Add(3,4) == 7` 화면에 출력**
- [x] `.csproj` 에 DLL 자동 복사 설정 추가
- [x] 프로젝트 속성 → 디버그 → **네이티브 코드 디버깅 사용 체크**
- [x] F10 으로 C# → C++ 함수 안으로 진입되는지 확인 (F11 대신 F10 사용 — 이유는 DEVLOG 참고)

**완료 조건:** C# 버튼 눌러서 C++ 함수가 계산한 값이 화면에 뜬다 ✅
**메모:** `pch.h` include 누락, P/Invoke 코드를 클래스 밖에 작성한 구조 오류,
시작 프로젝트가 `VisionCore`로 잘못 잡혀있던 문제 등 예상보다 다양한 지점에서 걸렸으나
로드맵이 경고한 "최대 고비"치고는 하루 안에 전부 해결.

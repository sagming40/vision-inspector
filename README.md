...
> **상태:** ✅ M3 완료 (C++ ↔ C# 배열/포인터 전달, 이미지 반전 성공) → 🚧 M4 진행 대기 — 2026.07
...
> M1 완료 — Python 단독 기준선 확보. C++ 비교치는 M4 완료 후 기입.

| 단계 | 구현 | fps |
|---|---|---|
| ④ 그레이스케일 | Python (cv2) | 2902.36 |
| ⑤ 블러 | Python (cv2) | 1422.61 |
| ⑥ 이진화 | Python (cv2) | 3157.20 |
| ⑦ 모폴로지 | Python (cv2) | 567.64 |
| ⑧ 블롭·면적 (A: 순수 반복문) | Python | 6.55 |
| ⑧ 블롭·면적 (B: numpy 벡터화) | Python | 0.44 |
...
> M2부터 `VisionApp` 빌드 시 `VisionCore.dll`이 자동으로 출력 폴더에 복사된다
> (`VisionApp.csproj`의 `AfterTargets="Build"` 설정).
